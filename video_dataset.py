"""
Adapted from: https://github.com/RaivoKoot/Video-Dataset-Loading-Pytorch/blob/d0903e07f527e3c8642d10c5ba6fe8b42e92f0da/video_dataset.py
"""
import os
import os.path
...
class VideoRecord:
    ...
    def transitions(self) -> List[int]:
        return [int(t) for t in self._data[3:][::2]]

    @property
    def labels(self) -> List[int]:
        return [int(l) for l in self._data[4:][::2]]

    def get_frame_label(self, frame_num: int) -> int:
        for i, t in enumerate(self.transitions):
            if frame_num <= t:
                return self.labels[i]
        raise RuntimeError(f'Cannot find {frame_num} in {self.path}')
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        for start_index in frame_start_indices:
            ...
            for _ in range(self.frames_per_segment):
                ...
                images.append(image)
                labels.append(record.get_frame_label(frame_index))
